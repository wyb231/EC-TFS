...
from utils import get_batch_data2
...
class CapsNet(object):
    def __init__(self, is_training=True, height=28, width=28, channels=1, num_label=10):
        ...
        with self.graph.as_default():
            if is_training:
                self.X, self.labels = get_batch_data2(cfg.dataset, cfg.batch_size, cfg.num_threads)
    ...
    def build_arch(self):
        with tf.variable_scope('Conv1_layer'):
            ...
            print("卷积层input:::", self.X, self.height, self.width)
            input = tf.layers.Input(self.X.shape[1:])
            conv1 = tf.contrib.layers.conv2d(self.X, num_outputs=256,
                                             kernel_size=9, stride=1,
                                             padding='same')
            ############################### 注意力机制 ###############################
            conv1 = self.ca(conv1) * conv1
            conv1 = self.sa(conv1) * conv1
