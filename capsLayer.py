...
class CapsLayer(object):
    ...
    Returns:
        A 4-D tensor.
    '''
    def __init__(self, num_outputs, vec_len, with_routing=True, channels=1, layer_type='FC'):
    ...
    def __call__(self, input, kernel_size=None, stride=None):
        ...
        if self.layer_type == 'CONV':
            ...
            if not self.with_routing:
                ...
                print("capsLayer.py胶囊层input:::", input)
                ...
                #                                    self.kernel_size, self.stride,padding="VALID",
                #                                    activation_fn=None)
                print("*************capslayer68.capsules***************", capsules.shape)
                # capsules = self.ca(capsules)
                # print("*************capslayer70.capsules***************", capsules.shape)
                # capsules = self.sa(capsules)
                # print("*************capslayer72.capsules***************", capsules.shape)
                capsules = tf.reshape(capsules, (cfg.batch_size, -1, self.vec_len, 1))
                # return tensor with shape [batch_size, 1152, 8, 1]
                capsules = squash(capsules)
                print("after squash capsules", capsules)
                return(capsules)
        ...
        if self.layer_type == 'FC':
            if self.with_routing:
                ...
                print("CapsLayer81",input.shape,input.shape[-2],input.shape[-2].value)
...
def routing(input, b_IJ, num_outputs=10, num_dims=16):
    ...
    print("capsLayer.py胶囊层W:::", W.shape)
    biases = tf.get_variable('bias', shape=(1, 1, num_outputs, num_dims, 1))
    print("capsLayer.py胶囊层biases:::", biases.shape)
    ...
    print("capsLayer.py胶囊层u_hat:::", u_hat.shape)
    ...
    for r_iter in range(cfg.iter_routing):
        with tf.variable_scope('iter_' + str(r_iter)):
            ...
            print("capsLayer.py胶囊层c_IJ:::", c_IJ)
